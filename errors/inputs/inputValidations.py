...
def validation_data_menu(empresa, mes, year):
    try:
        empresa = int(empresa)
        mes = int(mes)
        year = int(year)
        if validate_range_main(empresa, mes, year):
            return True
        else:
            raise inputError()
...
def validate_range_main(empresa, mes, year):
    return empresa > 0 and empresa < 6 and mes > 0 and mes < 13 and year > 0 and year < 3
...
def validation_bank_menu(input):
    try:
        input = int(input)
        if validate_range_bank(input, 4):
            return True
        else:
            raise inputError()
    except inputError as error:
...
def validate_range_bank(input, number):
